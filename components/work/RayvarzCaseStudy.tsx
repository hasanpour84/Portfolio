...
export function RayvarzCaseStudy() {
  ...
  return <>
    <section className="rayvarz-challenges" aria-labelledby="challenges-title"><div className="rayvarz-challenges__header"><h2 id="challenges-title">Challenges</h2><p>Six connected problems shaped the experience. Each required its own answer, while still feeling like one clear and confident Rayvarz journey.</p></div><div className="challenge-cards">{challenges.map(([title, description], index) => <article className="challenge-card" key={title}><Image src={challengeImages[index]} alt="" fill sizes="(max-width: 700px) 50vw, 33vw" /><span className="challenge-card__number">{String(index + 1).padStart(2, "0")}</span><div className="challenge-card__content"><h3>{title}</h3><p>{description}</p></div></article>)}</div></section>
