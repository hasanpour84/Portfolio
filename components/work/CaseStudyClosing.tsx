...
import styles from "./CaseStudyClosing.module.css";
...
export function CaseStudyClosing({ caseStudy }: { caseStudy: CaseStudy }) {
  ...
  return <>
    ...
    <section className={styles.caseClosing} aria-labelledby="outcome-title"><div className={styles.caseGallery}><figure><Image src={caseStudy.gallery[0]} alt={`Design detail from ${caseStudy.title}`} fill sizes="(max-width: 700px) 100vw, 50vw" /></figure><figure><Image src={caseStudy.gallery[1]} alt={`Team collaborating on ${caseStudy.title}`} fill sizes="(max-width: 700px) 100vw, 50vw" /></figure></div><section className={styles.results}><h2 id="outcome-title">Measured Outcome</h2><div className={styles.resultList}>{caseStudy.results.map(([value, label]) => <div key={label}><strong>{value}</strong><span>{label}</span></div>)}</div></section></section>
